...
#Create the special class that we are going to use from here on predict new data
class Absenteeism_Model(): 
    def __init__(self, model_file, scaler_file):
        #read the 'model' and 'scaler' files which were saved in previous step
    ...
    def load_and_clean_data(self, data_file):
        ...
        #store the data in a hew variable for later use
        self.df_with_predictions = df.copy()
        #drop the 'ID' column
        df = df.drop(['ID'], axis=1)
        #to preserve the code we've ccreated in the previous section, we will add a column with 'NaN' strings
        df['Absenteeism Time in Houses'] = 'NaN'
        
        #create a seperate dataframe, containing dummy values for all avaliable reasons
        ...
        #note: there's a more universal version of this code, however the filling will best suit our current purposes.
        columns_list = ['Date', 'Transportation Expense', 'Distance to Work', 'Age',
       'Daily Work Load Average', 'Body Mass Index', 'Education',
       'Children', 'Pets', 'Absenteeism Time in Hours', 'reason_1', 'reason_2', 'reason_3', 'reason_4']
        df.columns = columns_list
        
        #reorder the column name in df
       ...
       'Children', 'Pets', 'Absenteeism Time in Hours' ]
        ...
        column_names_upd = ['reason_1', 'reason_2', 'reason_3', 'reason_4', 'Month Value', 'Day of the Week', 'Transportation Expense', 'Distance to Work', 'Age',
       'Daily Work Load Average', 'Body Mass Index', 'Education',
       'Children', 'Pets', 'Absenteeism Time in Hours' ] 
        ...
        #drop the variables we decided we don't need
